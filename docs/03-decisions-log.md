...
### D054 — Magnetometer selected: Bosch BMM150 on a dedicated I2C bus

**Decision:** The magnetometer is the Bosch Sensortec **BMM150** (3-axis geomagnetic sensor, I2C/SPI breakout), placed **alone on its own dedicated I2C bus** (a second I2C peripheral, not the MS5611 bus). Canonical types gain `mag_status_t` (mirrors `imu_status_t`), `mag_reading_t { float field_ut[3]; uint32_t timestamp_us; mag_status_t status; }`, and `mag_healthy` in `health_flags_t`; `AVIONICS_PROTOCOL_VERSION` bumps 1 → 2 (the new health field changes `telemetry_frame_t`'s wire layout). No driver yet — the hardware-layer mag driver is task 7. Resolves R-YAW-01; reconciles the constraints §5 magnetic-disturbance failure mode with the hardware.
**Rationale:** The selected IMUs (BMI160, ICM-42688-P) are 6-axis — gravity pins roll/pitch but nothing references heading, so yaw drifts unbounded on the gyro alone (R-YAW-01). A magnetometer restores an absolute yaw reference, making yaw an observable, bounded EKF state, and **reopens the magnetic-disturbance failure mode** that §5 lists but the prior hardware could not exhibit — directly serving the fault-tolerance depth axis. BMM150 chosen on: real Bosch datasheet (noise density, Allan/bias specs) feeding LR-2's justified `R` — no magic constants; full-scale ±1300 µT (x,y) / ±2500 µT (z) gives headroom so a near-field metal disturbance yields valid-but-wrong readings rather than a railed sensor (the demo needs the gate to catch bad data, not saturation); I2C address 0x10/0x13 — no collision with the MS5611 (0x76/0x77). **Dedicated I2C bus, not shared with the MS5611:** preserves D026's isolation property (one device per bus → an I2C hang loses only that channel), so a mag-bus hang cannot also take the barometer, and keeps the slow mag off the time-critical SPI/IMU path. Cost is ~2 GPIO + a second I2C init — cheap on the 144-pin F407ZGT6. Two caveats, logged as considered tradeoffs, not oversights: (1) BMM150 is Bosch, same family as IMU-1 (BMI160) — minor because the mag is non-redundant (no mag-vs-mag cross-check; it is innovation-gated against the estimator's *predicted* field, not another sensor), but a Bosch-family common-mode could degrade IMU-1 and the mag together; FDIR degrades gracefully (isolate both → IMU-2 + dead-reckoned yaw). (2) BMM150 raw output requires Bosch trim-register compensation to yield true µT — a task-7 driver cost vs LIS3MDL's straight sensitivity scale. **Confirmation note for procurement:** the module must be a *standalone* BMM150, **not a BMX160** (BMI160+BMM150 on one die), which would hard-couple the mag with IMU-1 and defeat the isolation rationale.
**Alternatives considered:**
- **LIS3MDL (ST)** — strong alternative: simpler driver (direct sensitivity scale, no trim compensation), dissimilar silicon from both IMUs, I2C+SPI, ±16 gauss. A genuine close call; BMM150 chosen on UAE procurability and full-scale headroom. Retained as the fallback if BMM150 sourcing or the standalone-vs-BMX160 check fails.
- **QMC5883L / GY-271** — cheapest and most ubiquitous, but thin datasheet (poisons LR-2's justified-numbers rule), I2C-only (would force the shared-bus coupling), and clone-quality variance. Rejected as primary; acceptable last-resort fallback only.
- **MMC5983MA (Memsic)** — best raw noise, but a set/reset coil adds driver complexity and it is more exotic to source. Rejected on pragmatism for the accuracy we need.

## 2026-06-17 (airframe)

### D055 — Airframe segmentation and section-to-section joint standard

**Decision:** The airframe is built as a stack of identical-cross-section printed cylindrical **sections** with a uniform internal joint standard. Section geometry: length **280 mm**, OD **174 mm**, bore **150 mm**, wall **12 mm**, internal flange radial width **15 mm** (flange opening Ø120). The outer skin is a **continuous, fastener-free surface** — every fastener is internal. Sections join via an **internal twin-flange bolted joint**: the lower section presents a **thick flange** (15 mm boss, heat-set brass inserts), the upper section a **thin flange** (screw clearance holes); **8 × M3 screws on a 67.5 mm pitch-radius ring carry all axial clamp load**, generated by the screw head bearing on the thin flange (never by bottoming). The thick-flange hole is a **stepped pilot** (Stage 1 = insert pilot Ø, depth = insert length; Stage 2 = smaller screw-clearance Ø) whose diameter-step shoulder is the insert's repeatable bottom datum and gives screw-tip clearance. **4 × CNC solid-aluminium stringers** (Ø5 bores at R81, at 12/3/6/9 o'clock; 200 mm rods) bridge each joint, seated 100 mm into each section with the upper socket bored to **101 mm** (1 mm axial relief) so the stringers carry **no clamp load**. The datum hierarchy is one feature per job: **flange faces = axial**, **stringers = radial + clocking alignment**, **bolts = clamp**. No central spigot/register. Full reference: `docs/cad/airframe-architecture.md`; parametric insert/clamp formulas in `docs/cad/aft-most-section.md`.
**Rationale:** The ~300 mm print build volume caps section length, forcing a segmented airframe. An internal joint preserves the continuous outer skin (presentation requirement). Splitting the joint's three jobs onto three features avoids over-constraint: a single feature for axial, one for radial/clocking, one for clamp, so two separately-printed sections actually assemble. The stepped insert pilot solves heat-set depth inconsistency (the shoulder is a hard depth datum) while keeping the screw from bottoming. Stringers are local to the joints (not full-length spars) to avoid machining ~1.8 m of aluminium and to keep mass down; mid-span bending is carried by the printed skin — a deliberate, documented stiffness trade. Metal stringers are in scope per D044.
**Alternatives considered:**
- External band couplers / external fasteners — rejected; breaks the continuous fastener-free skin.
- Printed-in full-length stringers (true semi-monocoque) — rejected here; print-orientation and mass penalties, and the joint needs a discrete metal bridge anyway.
- Full-length solid-aluminium spars — rejected; machining cost and mass for no bench-load benefit.
- Bonded/glued joints — rejected; not serviceable/reusable, and chemical bonding is out of scope (D031/D044).
- A central centring spigot/register on the flange — rejected as unnecessary; the four stringers already serve as the radial/clocking datum, so a spigot would re-introduce over-constraint.
**Cross-references:** D031, D044; `docs/cad/airframe-architecture.md`, `docs/cad/aft-most-section.md`, `hardware/cad/aft-most.step`. PLA creep under sustained clamp is an accepted, out-of-scope limitation for this bench-only sub-month article (airframe doc §10).

### D056 — Control surfaces: four static aft fins + four actuated canards (supersedes the actuated-surface choice in D030)

**Decision:** The airframe carries **eight aerodynamic surfaces in two sets**: (1) **four static aft fins** — fixed, non-actuated, 90° "+", at the aft section, carrying no PWM channel; (2) **four actuated canards** — forward, 90°, each driven by its own servo on its own dedicated PWM channel. **The canards are the entire control authority**: closed-loop attitude control, the runtime mixing matrix, flight/demo deflection limits, and the 3-of-4 actuator-fault reconfiguration all act on the canards. This **supersedes D030's choice of the aft fins as the actuated surfaces**; D030's fault-tolerance rationale (four independent actuated surfaces ⇒ survive a single actuator fault on the remaining three) is **preserved unchanged** and simply now applies to the canards.
**Rationale:** Separates stabilisation (passive aft fins) from control authority (active canards), which is a cleaner and more aerospace-authentic configuration than all-moving aft fins, and gives a visually distinct control-surface set for the demo. The fault-tolerance depth axis is untouched — four independently actuated canards still leave three after a single fault, the minimum for three-axis moment control, and the mixing-matrix reconfiguration story holds verbatim. Requirements re-pointed: REQ-SYS-005 (now actuated canards), new REQ-SYS-013 (static aft fins), REQ-CTL-002/003/004/005/006, REQ-FDR-001/007/009, and the REQ-SYS-010 / REQ-GCS-005 wording.
**Alternatives considered:**
- Actuated aft fins, no canards (original D030) — superseded; conflates stabilisation and control on one surface set.
- All-moving aft fins — rejected; same conflation, and larger actuated surfaces for no fault-tolerance gain.
- Both canards and aft fins actuated (eight servos) — rejected; doubles the actuator/servo cost and power draw without earning additional fault-tolerance story beyond 3-of-4.
**Cross-references:** D030 (superseded on actuated-surface choice; rationale retained), D027-family power topology (servo rail), `docs/cad/airframe-architecture.md` §7.

### D057 — Avionics bay as a detachable self-contained capsule; dedicated 5 V servo rail reaffirmed

**Decision:** The avionics is packaged as a **self-contained, detachable capsule** — its own subassembly carrying the MCU, both IMUs, the magnetometer, the barometer, the radio, batteries, power conditioning, and all mounting points — housed in the **forward (avionics) section** before the nose. The capsule is built and bench-tested standalone, then dropped into the section and secured by an internal fastening mechanism; the section carries **service panels** for in-place servicing without nose removal. Servo power and PWM are connected after the capsule is seated. The canard servos run on a **dedicated 5 V rail, separate from the MCU's 3.3 V rail** (the servo rail is Rail B per D033 / REQ-PWR-004; this entry reaffirms that the isolation now applies to the canard servos).
**Rationale:** A standalone capsule makes the avionics independently buildable, testable, and serviceable, and gives every component a defined mounting point — the right modularity boundary for a system whose avionics is the primary deliverable. The dedicated servo rail prevents servo-slew current transients (several amps across four servos) from browning out the MCU or injecting ADC-reference noise that FDIR would misread as sensor faults — a structural fix consistent with the existing power topology.
**Alternatives considered:**
- Avionics bolted directly to the airframe interior (no capsule) — rejected; poor serviceability, no standalone test path, couples avionics build to airframe build.
- Shared 3.3 V / 5 V rail for MCU and servos — rejected; brown-out and FDIR-false-positive risk (already rejected by D033's Rail A / Rail B split).
**Cross-references:** D025, D032 (Pi/MCU role boundaries), D033 / REQ-PWR-004 (Rail B servo rail), architecture §2.5 (power), `docs/cad/airframe-architecture.md` §6, §8. Open: capsule fastening, service-panel retention, nose mechanism, cable mating (airframe doc §12).

### D058 — Aluminium stringers removed; skin wall thinned 12 → 6 mm (revises D055)

**Decision:** The four CNC-aluminium stringers of D055 are **removed**. Joint bending and torsion are carried by the 8-bolt internal flange alone. The skin wall is thinned from 12 mm to **6 mm**, with the **150 mm bore preserved as a hard requirement** (reach-in access), so the **OD drops 174 → 162 mm**; the internal flange, insert bosses, servo brackets, and fin roots remain locally thick ("thin skin, thick features"). With the stringers gone the joint has no precise radial datum — radial centring is now loose (flange face + bolt clearance), seam steps are accepted and finished under primer/paint, and a **centring spigot/register is demoted to an exploratory TODO** (airframe doc §12.2) for the builder to explore before adopting. The first article is built as a **minimal stack** — avionics section / one empty section / aft-most section / cosmetic motor mount — to prove every hard interface (joint, capsule fit, canard mount, nose mating) on the fewest prints; additional empty sections are added later for length.
**Rationale:** The stringers were a flight-load instinct that bench loads (tens of N, ~tens of N·m) do not justify. The bolted flange reacts the worst bench moment with several-times margin before insert pull-out, and a 162 mm / 6 mm-wall tube is hugely over-stiff (section I ≈ 9×10⁶ mm⁴; ~0.4 mm tip deflection under 50 N at 0.9 m) — 12 mm of PLA at this diameter does not meaningfully bend, and the stringers' ~50% stiffness contribution sat on top of an already wild over-spec. Removing them and halving the wall cuts wall volume by well over half (mass, print time, filament — risk R-AF-03) and lightens the top-heavy CG, while deleting the stringer-bore coaxiality assembly risk entirely (R-AF-01 closed). The 12 mm wall was itself partly a consequence of embedding the Ø5 stringer bores; with the bores gone, denting/handling margin sets the wall, and 6 mm is a deliberately conservative ("still want to be safe") choice over the ~4 mm the stiffness math would allow.
**Alternatives considered:**
- Keep the stringers — rejected; unjustified mass/machining/assembly cost for bench loads, and the source of the coaxiality risk.
- Thin to ~3–4 mm — viable by the stiffness math but rejected for now in favour of a safer 6 mm; the new limiting mode is local denting, not bending, so margin there is cheap insurance.
- Keep OD 174 and open the bore to 162 — offered; rejected in favour of preserving the 150 mm bore (the stated hard access requirement) and taking the OD reduction.
- Add a centring spigot now — deferred to an exploratory TODO; it is the clean radial datum once stringers are gone (no longer over-constrained), but the builder wants to explore its geometry/assembly before committing.
**Cross-references:** revises D055 (stringer + wall-thickness portions; the joint clamp/flange/insert scheme stands); `docs/cad/airframe-architecture.md` §3–§5, §12.2; risk register R-AF-01 (closed), R-AF-03 (downgraded). The companion `aft-most-section.md` and `aft-most.step` predate this and carry the superseded OD/wall/stringer figures.

### D059 — Control surfaces have no aerodynamic authority on the static bench; demo is commanded-response

**Decision:** It is recorded as a scope clarification that the canards (and fins) exert **no aerodynamic moment on the static test stand** — there is no airflow. The demo is therefore **commanded-response, not closed physical control**: a manual perturbation of the stand produces an attitude error, the estimator senses it, and the canards throw in the correct restoring sense, with **demo mode deliberately exaggerating the deflection** (REQ-CTL-004) for visible "violent" effect. The verifiable behaviour is the correct *commanded* canard response (sign/magnitude vs. estimated error) plus the live estimator/health telemetry — not vehicle motion. `REQ-CTL-001` and `REQ-CTL-006` are reworded to govern the command-path response, with physical attitude restoration explicitly out of scope on the static bench.
**Rationale:** Honesty to the bench setup (Constraints §10 spirit): TVC is excluded and there is no flow, so aerodynamic surfaces cannot move the article. Writing CTL-001/006 as "attitude returns to setpoint" made a physically unrealisable claim and an unverifiable requirement. Reframing to the commanded response keeps both requirements verifiable, preserves the GNC story (estimator → control law → correct actuator command under fault), and matches what the audience actually sees. The exaggerated demo-mode throw is a feature, not a defect — it makes the (otherwise authority-free) reaction legible.
**Alternatives considered:**
